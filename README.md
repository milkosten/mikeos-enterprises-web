...
Responses are cached in-memory for 60 s; backend calls time out at 6 s so a slow
registry can never hang the page. Each country degrades independently (error
chips, not a blank page).
...
`.env` on the server (mode 0600) provides `OSM_TOKEN`. Caddy
